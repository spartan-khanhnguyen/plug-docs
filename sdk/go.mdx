...
require (
    github.com/plugnpay/plugnpay-go v1.0.0
)
```
...
func main() {
    ...
    client, err := plugnpay.NewClient(&plugnpay.Config{
...
```go
import "github.com/plugnpay/plugnpay-go"
...
session, err := client.CreateCheckoutSession(ctx, plugnpay.CreateCheckoutSessionArgs{
    ...
    Provider:   plugnpay.ProviderStripe,                 // Default: ProviderStripe
...
func (s *PlugnPayService) CreateCustomerAndCheckout(ctx context.Context, email, productID string) CheckoutResult {
    ...
    customer, err := s.client.GetCustomer(ctx, plugnpay.GetCustomerArgs{
        CusID: "customer_123",
        Email: email,
    ...
    checkout, err := s.client.CreateCheckoutSession(ctx, plugnpay.CreateCheckoutSessionArgs{
        CustomerID: customer.CustomerID,
        ProductID:  productID,
        SuccessURL: "https://yourapp.com/success",
        CancelURL:  stringPtr("https://yourapp.com/cancel"),
        Provider:   plugnpay.ProviderStripe,
    })
    if err != nil {
...
func (s *PlugnPayService) CheckUserAccess(ctx context.Context, customerID, featureID string) AccessResult {
    // 3. Check Access
    access, err := s.client.CheckAccess(ctx, plugnpay.CheckAccessArgs{
    ...
    _, err = s.client.TrackUsage(ctx, plugnpay.TrackUsageArgs{
        CustomerID:     customerID,
        FeatureID:      "api-calls",
        Usage:          1,
    ...
    usage, err := s.client.GetUsage(ctx, plugnpay.GetUsageArgs{
        CustomerID: customerID,
        FeatureID:  "api-calls",
    })
    if err != nil {
...
func (s *PlugnPayService) handleEvent(event plugnpay.Event) {
    switch event.EventType {
    case "subscription.created":
        var payload plugnpay.SubscriptionEventData
        if err := json.Unmarshal(event.Data, &payload); err != nil {
