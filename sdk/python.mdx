...
The PlugnPay Python SDK provides a comprehensive, async-first interface for integrating payment processing, subscription management, and real-time event streaming into your Python applications. Built with modern asyncio patterns and type safety, it offers seamless integration with PlugnPay's complete platform.
...
async def main():
    ...
    async with PlugnPay() as client:
        customer = await client.get_customer("cus_123", email="user@example.com")

    # Using direct API key
    async with PlugnPay(api_key="pk_test_xxxxxxxxxx") as client:
        customer = await client.get_customer("cus_123", email="user@example.com")
...
```python
from plugnpay import PlugnPay, PlugnPayConfig

config = PlugnPayConfig(
    api_key="pk_test_xxxxxxxxxx",
    base_url="https://api.plugnpay.dev",  # Custom base URL
    timeout=30.0,  # Request timeout in seconds
    user_agent="MyApp/1.0.0",  # Custom user agent
)

async with PlugnPay(config=config) as client:
...
```python
from plugnpay.types import SSEEvent
...
async def comprehensive_error_handling():
    async with PlugnPay() as client:
        try:
            ...
            print(f"General PlugnPay error: {e}")
...
```python
import asyncio
from plugnpay import PlugnPay, Provider
...
This completes the PlugnPay Python SDK documentation. The SDK provides a powerful, type-safe, and async-first interface for all your payment and subscription management needs.